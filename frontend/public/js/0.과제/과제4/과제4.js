...
function newClick( ){
	...
	if( newBook.length>4 ){
			if( newBook.length<11 ){
		
				도서목록.push(newBook)
			 
			}else{ alert('5~10자 사이의 도서명을 입력하세요.')
...
function 고객테이블(){
	...
	for(let i = 0 ; i < 도서목록.length ; i++  ){
		bookTable2 += `<tr> 
						...
						<th> ${ 도서목록[i]} </th> 
						<th> <div class="checkOut"></div> </th> 
						<th> <button class="대여버튼"  >대여</button><button class="반납버튼" >반납</button> </th> 
					</tr>`
				
	}
	document.querySelector('.bookList').innerHTML = bookTable2
	document.querySelector('.대여버튼').style.display = 'none'
	document.querySelector('.반납버튼').style.display = 'none'
