	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 4. 파일 스트림[바이트단위] *예외발생 서블릿이 자동으로 예외처리
			// 1. 파일 객체의 바이트를 모두 읽어온다.
					 // 1. 해당 경로 [파일] 스트림객체 만들기 -- [스트림이 서버파일과 연결됨] 들어오기만 가능한 도로
					BufferedInputStream fin = new BufferedInputStream( new FileInputStream(file));
					// 2. 스트림객체에서 꺼내올 바이트들을 저장할 바이트 배열 준비 [선언] 
					// 바이트배열 길이 = 파일의 길이[바이트 길이]
					byte[] bytes = new byte[ (int)file.length() ];
					// 3.파일입력스트림객체에서 read() : 해당 파일을 바이트로 읽어오는 함수
					fin.read(bytes); // -- byte배열을 읽어옴
			// 2. 읽어온 바이트를 모두 출력한다. [ 클라이언트 에게 응답 response ]
					// 1. response응답객체에서 출력스트림 호출해서 파일출력스트림객체 만들기
					//response.getOutputStream() : HTTP 스트림 단위[바이트] 전송 -- 유저랑 response로 스트림도로깔음 / response: http 유저한테 응답할수있는방법 나가기만 가능한 도로
					// response.getOutputStream() 바이트로 내보내는데 write함수가 없어서 / 나갈수있는도로 어디에? response.getOutputStream() 유저한테
					// response.getOutputStream() 도로를 버퍼로 재포장
