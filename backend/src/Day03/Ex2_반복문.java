package Day03;

public class Ex2_반복문 { // class s
	
	public static void main(String[] args) { // main s
		
		//	[ p.124 ]
		int sum = 0;
		sum = sum + 1 ;
		sum = sum + 2 ;
		sum = sum + 3 ;
		sum = sum + 4 ;
		sum = sum + 5 ;
		
		// for ( 1. 초기값 ; 2.조건식 ; 4.증감식 ){ 3. 실행문 }
		for( int i = 1 ; i <= 5 ; i++ ) { sum = sum + i; }
			// i가 1부터 5까지 1씩증가할 때마다 실행문 실행
		
		
		
		// [ p.125 ] 1~10 출력
		
		for ( int i = 1 ; i <= 10 ; i++ ) {
			// i가 1부터 10까지 1씩 증가할때마다 실행문 실행
			System.out.println( i );
		}
		
		// [ p.126 ] 1~100 누적합계
		int total = 0;
		for ( int i = 1 ; i <= 100 ; i++ ) {
			// i가 1부터 100까지 1씩증가할때마다 실행문 실행
			System.out.println( i + " " );
			total += i ; // total = total + i
			
		}
		System.out.println("\n총누적합계 : " + total);
		
		
		
		// [p127] 0.1 ~ 1.0 사이의 실수
		for ( float x = 0.1f ; x <= 1.0f ; x += 0.1f ) {
			 // x는 0,1 부터 1.0까지 0.1씩 증가할때마다 실행문 실행
			System.out.println( x + " ");
		}
		
		
		
		// [p127~128] for중첩 : 구구단
			// 단 : 2 3 4 5 6 7 8 9
			// 곱 : 1 2 3 4 5 6 7 8 9
				/*
				  곱은 단마다 반복!!
				  2*1	3*1
				  2*2	3*2
				  2*3
				  2*4
				  2*5 
				 
				 [ 단 8회 , 곱 9회 ] 8*9 총 회전수 72회
				 * 
				 */
		System.out.println();
		for ( int 단 = 2 ; 단 <= 9 ; 단++ ) {
			System.out.println( "단 : " + 단);
			// 단이 2부터 9까지 1씩 증가할때마다 아래 포문 실행
			for ( int 곱 = 1 ; 곱 <= 9 ; 곱++) {
				System.out.println("    곱 : " + 곱);
				System.out.println( "\t\t" + 단 + " X " + 곱 + " = " + (단*곱));
			}
		}
	
		// 6. [ p.134 ] break; 반복문탈출  return; 함수종료
		while ( true ) { // 무한루프 , 무한반복
			int num = (int)(Math.random()*6)+1;
			if ( num == 6 ) {
				System.out.println("while{ }탈출");
				break; // 가장 가까운 반복문 탈출
			}
		}
		
		// 7. [p.135] 나가고싶은 for문 지정
		
		// 이름 : for( ){ }
		// break 이름;
		
		// 1번 for문
		첫번째for : for( char upper = 'A'; upper <= 'Z' ; upper++ ) {
				// A부터 Z까지 문자 하나씩 증가 반복
				System.out.println( upper );
				System.out.println();
				
				// 2번 for문
				두번째for : for ( char lower = 'a' ; lower <= 'z' ; lower++ ) {
					
					System.out.println( "\t"+lower );
					if( lower == 'g') {
						//만약에 소문자 g 이면
						// break; // 가장가까운 반목문 탈출 for2 탈출
						break 첫번째for; // 특정 for 탈출
						
					}
				}
							
		}
		
		// 8. [p.136] continue
					forname : for ( int i = 1 ; i <= 10 ; i++ ) {
							if ( i%2 != 0 ) {// i가 홀수이면
								// continue  // 가장 가까운 반복문[증감식]으로 이동
								continue forname; // 특정한 반복문[증감식]으로 이동
							}
							System.out.println( i + " ");
						}
		
	}
	
	
	
} // class e
